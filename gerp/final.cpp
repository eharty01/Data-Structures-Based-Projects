...
string line_content) {
    ...
    // If not already stored, insert the new ValueInfo
    // if (not stored_already) {
    //     ValueInfo new_value_info(path, line_num, line_content, word);
    //     my_hash.at(index).push_back(new_value_info);
    //     numItems++;
...
ofstream &outputFile) {
    ...
    for (const ValueInfo &valueinfo : values_list) {
        if (i_search) {
            ...
            if (key == stored) {
                //match_found = print_insensitive(key, outputFile);
                match_found = true;
                outputFile << valueinfo.file_path << ":" << valueinfo.line_num;
                outputFile << ": " << valueinfo.line_content << endl; 
            }
...
// bool Command::print_insensitive(string key, ofstream &outputFile) {
//     bool result = false;
...
/*
bool Command::print_sensitive(string processed_word, ofstream &outputFile) {
    for (const ValueInfo &valueinfo : values_list) {
        string stored = valueinfo.original_word;
        if (processed_word == stored) {
            outputFile << valueinfo.file_path << ":" << valueinfo.line_num;
            outputFile << ": " << valueinfo.line_content << endl;
        }
    }
